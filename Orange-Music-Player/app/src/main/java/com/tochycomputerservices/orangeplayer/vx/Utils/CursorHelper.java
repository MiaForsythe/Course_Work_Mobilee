...
//            int i =cursor.getColumnIndex(MediaStore.Audio.Media.GENRE);
//            String g=cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.GENRE));
//            Long l = cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.Media.GENRE_ID));
//            Long k = cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.Media.GENRE_KEY));
            do {
                Song song = new Song(
...
*/
            ArrayList<Song> songs = getTracksForSelection("SONGS", "");
            ...
            try {
              ...
              /*  if (cursor != null && cursor.moveToFirst()) {
                    do {
                        ...
                        genre.put(DataBaseHelper.GENRE_ID, ""+cursor.getInt(0));
                        genre.put(DataBaseHelper.GENRE_NAME, cursor.getString(1));

                        ArrayList<Song> songs = getTracksForSelection("GENRES", ""+cursor.getInt(0));
                        int cnt=0;
                        ArrayList<Long> albums_ids = new ArrayList<>();
                        for (int i=0;i<songs.size();i++) {

                            if(!albums_ids.contains(songs.get(i)._albumId)){
                                albums_ids.add(songs.get(i)._albumId);
                            }
                        }
                        String albums_ids_str=albums_ids.toString();

                                ArrayList<Album> albums = getAlbumsForSelection("GENRES", albums_ids_str);
...
//                }
    ...
    private static boolean shouldBeScanned() {
        if (PreferencesHelper.getInstance().getBoolean(PreferencesHelper.Key.FIRST_LAUNCH, true)) {
            ...
            int scanAt = Integer.parseInt(sharedPreferences.getString("preference_key_scan_frequency", "0"));
